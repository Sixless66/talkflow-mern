  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
       ...
       {/* ------ head ------ */}
        ...
        <div className='flex flex-col h-[calc(100%-120px)] overflow-y-sroll p-3
        pb-6'>
               {
                messagesDummyData.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 
                      justify-end ${msg.senderId !== '664' && 'flex-row-reverse'}`}>
                           {
                          msg.image ? (
                            ...
                            rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.
                              senderId === 'gha' ? 'rounded-br-none' : 'rounded-bl-none'}`}>
                              {msg.text}
                            </p>
                          )}

                          <div className='text-center text-xs'>
                               <img src={msg.senderId === 'fhd' ? assets.avatar_icon
